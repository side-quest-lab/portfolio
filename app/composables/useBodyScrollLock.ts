import { createSharedComposable, isClient, isIOS, useEventListener, type Fn } from "@vueuse/core";
...
const useBodyLockStackCount = createSharedComposable(() => {
  ...
  watch(
    ...
    (val, oldVal) => {
      ...
      const config = { padding: 0, margin: 0 };

      if (verticalScrollbarWidth > 0) {
        document.body.style.paddingRight =
          typeof config.padding === "number" ? `${config.padding}px` : String(config.padding);
        document.body.style.marginRight =
          typeof config.margin === "number" ? `${config.margin}px` : String(config.margin);
