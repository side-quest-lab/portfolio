...
<template>
  ...
  >
    <FeaturesHeroCanvas class="absolute inset-0 z-0" />

    <div class="relative z-10 max-w-7xl mx-auto px-6 w-full">
      <span class="hero-number text-sm text-foreground/30 font-dm-mono block mb-8"> 01 </span>
        ...
        <div class="flex flex-col items-center lg:items-end gap-6">
          ...
          >
            <span class="font-space-grotesk text-3xl md:text-4xl font-bold text-foreground/40">
              {{ info.firstName[0] }}{{ info.name.split(" ").pop()?.[0] }}
            </span>
          </div>
